...
export default function Trips() {
  ...
  useEffect(() => {
    fetchTrips();
    api.get('/vehicles').then(({ data }) => setVehicles(Array.isArray(data) ? data : data.vehicles || []));
    api.get('/drivers').then(({ data })  => setDrivers(Array.isArray(data)  ? data : data.drivers  || []));
  }, [fetchTrips]);
  ...
  function vehicleName(v) {
    if (!v) return '—';
    if (typeof v === 'object') return `${v.name || ''} (${v.registrationNumber || ''})`;
  ...
  return (
    <div>
          ...
          <form onSubmit={handleCreateTrip}>
            <div className="form-grid">
              <div className="form-group">
                <label>Source</label>
                <input name="source" value={form.source} onChange={handleChange} placeholder="e.g. Bangalore" required />
              </div>
              <div className="form-group">
                <label>Destination</label>
                <input name="destination" value={form.destination} onChange={handleChange} placeholder="e.g. Chennai" required />
              </div>
              <div className="form-group">
                <label>Vehicle</label>
              ...
              <div className="form-group">
                ...
                <input name="cargoWeight" type="number" value={form.cargoWeight} onChange={handleChange} placeholder="e.g. 2000" />
              </div>
              <div className="form-group">
                <label>Planned Distance (km)</label>
                <input name="plannedDistance" type="number" value={form.plannedDistance} onChange={handleChange} placeholder="e.g. 350" />
              </div>
            </div>
      ...
      {completeModal && (
        <div className="form-panel" style={{ borderColor: 'rgba(34,197,94,0.3)' }}>
          <h3>Complete Trip</h3>
          <form onSubmit={handleComplete}>
            ...
            <div className="form-actions">
              <button type="submit" className="btn btn-success">Confirm Complete</button>
                  ...
                  <td>
                    <div className="action-group">
                      {t.status?.toLowerCase() === 'draft' && (
                        <button className="btn btn-sm btn-warning" onClick={() => handleDispatch(t._id)}>
                          🚀 Dispatch
                        </button>
                      )}
                      {t.status?.toLowerCase() === 'dispatched' && (
                        <>
                          <button
                            ...
                            onClick={() => { setCompleteModal(t._id); setActionError(''); }}
                          >
                            ✅ Complete
                          </button>
                          <button className="btn btn-sm btn-danger" onClick={() => handleCancel(t._id)}>
                            ✕ Cancel
                          </button>
                        </>
                      )}
                      {(t.status?.toLowerCase() === 'completed' || t.status?.toLowerCase() === 'cancelled') && (
                        <span style={{ color: 'var(--text-dim)', fontSize: 12 }}>No actions</span>
                      )}
                    </div>
                  </td>
                </tr>
